        ...
        <ul class="clients-track group" style="--gap: 2rem; --speed: 50s;">
          ...
          @foreach ($items as $i => $logo)
            <li class="clients-item">
              @php
                $img = '<img src="'.$logo['src'].'" alt="'.$logo['alt'].'" loading="lazy"
                         class="block object-contain clients-img"
                         style="height: 100px" />';
              @endphp

              @if (!empty($logo['url']))
                <a href="{{ $logo['url'] }}" class="inline-block px-10" aria-label="{{ $logo['alt'] }}"
                   @if($i >= count($logos)) aria-hidden="true" tabindex="-1" @endif>{!! $img !!}</a>
              @else
                <span class="inline-block px-10" @if($i >= count($logos)) aria-hidden="true" @endif>{!! $img !!}</span>
              @endif
            </li>
          @endforeach
        </ul>
